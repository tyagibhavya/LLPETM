...
// /*  
// track the realized or closed PnL for positions that have been closed (real_pnl_), 
// the unrealized or open PnL for the currently open position (unreal_pnl_), 
// the total PnL, which is a summation of the two values (total_pnl_) 

// open_vwap_ std::array variable tracks the product of price and execution quantity on each side
// when there is an open long (positive) or short (negative) position. We will need this to compute
// the unrealized PnL by comparing the volume-weighted average price (VWAP) of the open long or short
// position against the current market price.
// */
//   struct PositionInfo {
//     int32_t position_ = 0;
...
// /* Refer book -> Handling order executions in PositionInfo (Pg 377) */
...
// /* 
// We provide the BBO object that corresponds to the trading instrument that was updated in the updateBBO() method.
// We save the bbo argument provided in this method in the bbo_ data member in our PositionInfo struct. 
// This method only has anything to do if position_ is non-zero and the bid and ask price values on the BBO provided are valid. */
