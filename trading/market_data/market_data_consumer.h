...
/*
 chose the std::map type here since it is easier to iterate over sorted keys compared to, 
 say, std::unordered _map. Note that std::map is not efficient for a wide range of reasons – 
 the internal data structure is Red Black Tree, which has an asymptotic insertion performance of O(log(N)) 
 and causes dynamic memory allocations, etc. However, we make an exception in this case because 
 snapshot recovery is expected to be extremely rare, and when the MarketDataConsumer class is recovering from the snapshot stream, 
 trading is generally paused inside the client’s trading application
 since it does not have an accurate view of the state of the order book. Additionally, the snapshot stream is delayed 
 and throttled from the exchange’s side, so the snapshot synchronization process itself is not required to be low latency.
*/
...
/*  in_recovery_ boolean flag to signify if MarketDataConsumer detected a packet drop and 
is currently trying to recover using the snapshot and incremental market data streams */
    bool in_recovery_ = false;

    /// Information for the snapshot multicast stream.
/*  the multicast stream and network interface information in the iface_ variable, the snapshot_ip_ variable,
and the snapshot_port_ variable. These represent the network interface to use, the IP address, and the port of the snapshot multicast stream */
